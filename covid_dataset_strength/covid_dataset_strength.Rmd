...
```{r}
dna_evolution_rate_per_genome_per_year <- 26
```
...
was created in December 2019. Currenly, May 2020, 
...
There are `r n_sequences` COVID-19 sequences (`https://nextstrain.org/ncov`).
...
```{r}
p_state_transition <- 0.01
```

Based upon nothing but our gut instinct, we estimate this
chance at `r p_state_transition`.
...
Based upon nothing but our gut instinct, we base a conclusion on the
number of transitions as shown in the code above.

```{r}
conclusion <- draw_conclusion(n_state_transitions)
```

From that we conclude that for our expected `r n_state_transitions` state
transitions, that our dataset is `r conclusion`.
