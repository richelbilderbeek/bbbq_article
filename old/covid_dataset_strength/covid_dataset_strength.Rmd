...
```{r}
pureseqtmr::plot_topology(topology)
```

Extract data from the topology:
...
We will determine the number of unique sequences
from the percentage of unique sequence per protein.

To do so, we read all COVID-19 sequences:

```{r cache=TRUE}
all_sequences_filename <- "../downloads/allprot0621.fasta"
testthat::expect_true(file.exists(all_sequences_filename))
sequences <- seqinr::read.fasta(
  all_sequences_filename, 
  seqtype = "AA",
  as.string = TRUE
)
```

Grouping them by protein:

```{r}
t <- tibble::tibble(
  prot_name = as.character(
    na.omit(
      stringr::str_match(
        names(sequences),
        "^([:upper:][:alnum:]{0,5})\\|"
...
library(dplyr)
t$prot_name <- as.factor(t$prot_name)
t_unique <- t %>% group_by(prot_name) %>% 
  summarise(n = n(), n_distinct = n_distinct(sequence)) %>%
  mutate(perc = n_distinct / n)
perc_unique_protein <- mean(t_unique$perc)
```

Of all proteins, on average, `r perc_unique_protein` are unique.
We are looking, however, for unique proteomes. The percentage
of unique proteomes is the product of 

```{r}
percs_duplicate_protein <- 1.0 - t_unique$perc
perc_duplicate_proteome <- prod(percs_duplicate_protein)
perc_unique_proteome <- 1.0 - perc_duplicate_proteome
```

```{r}
n_unique_sequences <- perc_unique_proteome * n_sequences
```

Therefor, there are `r n_unique_sequences` unique sequences.

```{r}
n_tmh_aa_mutations <- tmh_aa_evolution_rate_per_sequence * n_unique_sequences
```
