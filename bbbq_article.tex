...
A human, viral and bacterial reference proteome was used to determine the percentages of epitopes overlapping with TMHs. The human reference proteome has UniProt ID UP000005640_9606, which is a representative set of the full proteome and also used in our previous study (\cite{bianchi2017}).  SARS-CoV-2 with reference proteome UniProt ID UP000464024 was used as a viral proteome and Mycobacterium tuberculosis (MTb) with UniProt ID UP000001584 was used . These pathogens were chosen based on their impact on the human population(references).
...
To determine if epitopes derived from TMHs are presented we used epitopes identified in \frans{Welke studies is dit dan van SARS-COV2 en MTB ?}
for MHC-I (\cite{schellens2015comprehensive}) 
and MHC-II (\cite{bergseng2015different})peptide elution studies.
For each of these peptides, the human representative reference proteome was searched for its origin\frans{wat bedoel je?}. We filtered for epitopes that were uniquely present in the proteome and correlated these for overlap with predicted transmembrane regions in the proteome. 
In this analysis, both TMHMM and PureseqTM (see section ’Prediction software used’) were used to predict the transmembrane topology. 
...
To determine the percentages of MHC-I and MHC-II epitopes overlapping with TMHs, we applied a similar analysis strategy as described in  \cite{bianchi2017}.
...
as found in the elution study (\cite{bergseng2015different}).\frans{what percentage is 9 mer or 14 mer?}
