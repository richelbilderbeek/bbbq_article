...
[GEERT: I removed the nr of MHC molecules at is is more complex, as the alpha chains coded by the maternal chromosome of HLA-DQ,DR,DP can heterodimeriza with the beta chains coded by the paternal chromosome and vice versa]
...
of the binding affinities of peptides (\cite{larsen2010identification,schellens2008unanticipated,tang2011genome}).[GEERT: can you put 'very reliable' in more quantitative terms (e.g. with overall more than 90\% accuracy)]
...
with high accuracy [GEERT: more quantitative?] by bioinformatics approaches (\cite{krogh2001predicting,bianchi2017,kall2004combined,arai2004conpred,jones2007improving,klammer2009metatm,wang2019efficient}). 
...
paragraph here?][GEERT: I think it is OK like Richel wrote]
...
\richel{This is Frans' text, merge it with the current text: 
...
\url{https://www.ncbi.nlm.nih.gov/snp/docs/RefSNP_about/}), and the databases 'gene' (for gene names)
and 'protein' (for proteins sequences). [GEERT: any references for databases gene and protein?]
...
%
SNPs were picked based on ID number, which is linked to their discovery date. To verify that ID number is unrelated to SNP positions the relative positons of the analyzed SNPS in a protein was determined and showed no positional bias, shown in figure \ref{fig:snp_rel_pos}.[GEERT: rood practice is to present the data in a chronological fashion: thus move this to figure 1 (or put in supplement)]

Per SNP, the 'protein' NCBI database was queried for the
protein sequence.
For each protein sequence, the protein topology was determined 
using PureseqTM.
Using these predicted protein topologies, the SNPs were scored to be located within or outside trans membrane domains. [GEERT: for Table 1: Tables don't have a legend, but instead have a title and footnotes below the table.]
...
[GEERT: I would move this subsection to below: first explain the bioinformatics predictions, and then show this as an experimental confirmation]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Prediction of TMH-derived peptide presentation in MHC-I}
[GEERT: mooier is je subheadings samen te vatten met een conclusie. Dus: TMH-derived peptides are predicted to be over-presented in MHC-I (en dan ook voor de andere subheadings)]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We recently predicted that peptides derived from TMHs of human proteins 
are over-presented on most haplotypes of HLA-A and HLA-B. Here, we wondered if this over-presentation is general 
and if peptides stemming from predicted TMHs of microbial pathogens 
would also be over-presented on HLA-A and HLA-B. We therefor analysed all superhaplotypes[GEERT: not explained, for instance in the Methods] of the HLA-A and HLA-B class (Figure \ref{fig:1}). Similar as for the human proteome, the percentages of MHC-I epitopes predicted to be overlapping with TMHs  was higher as expected by TMH abundance for the proteomes of SARS-CoV-2 and \emph{M. tuberculosis}.
See the supplementary materials (table \ref{tab:tmh_binders_mhc1}) [GEERT what is in supplementary table 6? The numbers?]
The extent of the over-presentation between the various different HLA supertypes is similar for the probed proteomes, which strenghtens our previous conclusion [ref] that the hydrophopbicity of the MHC-binding groove is the main factor responsible for the predicted over-presentation of TMH-derived peptides.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Prediction of TMH-derived peptide presentation in MHC-II}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
See \ref{fig:rel_presentation} and \ref{fig:rel_presentation_per_haplotype}. [GEERT: Wat is er in 9 en 10? Wat is de conclusie? Dit moet nog wat verder uitgewerkt worden]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Evolutionary conservation}
[GEERT: zie mijn punt hierbove. Een korte conclusie als titel is veel sterker. Dus iets als: TMHs are more evolutionary conserved.]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{figure}[!htbp]
  \includegraphics[width=\textwidth]{ncbi_peregrine_results/fig_conservation.png}
  \caption{
    Evolutionary conservation of SNPs in TMHs. [GEERT: legenad uitwerken. Wat is precies laten zien?]
  }
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    for either single- or multi-spanners/ [GEERT: ook uitwerken]
  }
...
\begin{figure}[!htbp]
  ...
  \caption{
    ...
    The red line [GEERT: where is the red line? Behind the blue line? That makes this useless and can be removed.]is a linear trend line that originates from the origin. 
...
as depicted in figure \ref{fig:conservation}. [GEERT: zie mijn eerder opmerking over panels. Ik zou met panel A, B, C, etc werken.] [GEERT: hieronder had je indentieke tekst, maar verwees naar andere figuur \ref{fig:conservation_per_spanner}. Ik denk dat je deze hieronder kan samenvoegen]
...
\subsection{Elution studies}
[GEERT: ook hier: veel sterker is een korte conclusie, of boodschap]

% \paragraph{TMH presentation in vivo}

Although elution studies and in silico predictions both suggest that TMH-derived epitopes are presented in MHC-I and MHC-II, the molecular mechanisms of how integral membrane proteins are processed for MHC presentation are largely unknown [GEERT: cite Bianchi]. Most prominently, the fundamental principles of how TMHs are extracted from the  hydrophobic lipid environment into the aqueous vacuolar lumen, and their prior or subsequent proteolytic processing are unresolved. [GEERT: beetje kort. Suggesties? Experimental approaches? Hoeft niet super origineel, bijvoorbeeld suggereren dat lipases een rol kunnen spelen. Frans kan vast helpen met mooie zin.]
...
However, two known biological phenomena that violate this assumption
...
by mass spectronomy [GEERT: referentie, zie paper Bianchi]. This means that the number of TMH-derived epitopes
...
The over-presentation of TMHs for MHC-II haplotypes 
is slight more pronounced than for MHC-I,
as with a third of the predicted binders 
\richel{Hier ben ik nu met verwerken...} [GEERT: dan stop ik hier eventjes? Of de rest ook weer keer lezen? Laat maar weten]
from the human and MtB proteome stem from TMH human reference proteome and MtB reference proteome to be presented on MHC-II. \frans{ik zoek nog naar associaties} 
