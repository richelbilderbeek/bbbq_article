...
In this study, we'll be using MHCnuggets, as it is the only FOSS tool available.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Strength of Influenza A}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Our evolutionary experiment depends on the strength of the dataset.
Here we do a back-on-the-envelope calculation to assess if the dataset
is strong enough.

\begin{itemize}
	\item Genome size: $\approx 1.3 \cdot 10^4$ bases
	\item Relative genome size compared to COVID-19: $\approx 0.4$ (as $\frac{1.3 \cdot 10^4}{3.0 \cdot 10^4}$)
	\item Full genome sets: $\approx 3.2 \cdot 10^4$ (Influenza A)
	\item Relative number of genome sets compared to COVID-19: $\approx 6.4$ as much (as $\frac{\approx 32 \cdot 10^3}{5 \cdot 10^3}$)
	\item Relative time to accumulate mutations: $\approx 12$ times longer (average age $\approx$ 4 year, before 2015, already 8k sequences)
	\item Relative chance of observing transitions: $\approx 30$ (as $0.4 \cdot 6.4 \cdot 12$)
	\item Expected number of observed TMH mutations in COVID-19 that cause a transition: $\approx 8$
	\item Expected number of observed TMH mutations that cause a transition: $\approx 240$ (as $8 \cdot 30$)
\end{itemize}

Conclusion: seems strong enough to us.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{EpitopePrediction versus MHCnuggets}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

EpitopePrediction is a recent tool, used in \cite{bianchi2017},
and can predict MHC-I epitopes.
MHCnuggets is an even newer tool to predict both MHC-I and MHC-II epitopes.

\begin{figure}[!htbp]
  \includegraphics[width=0.9\textwidth]{ep_vs_mhcn/ep_vs_mhcn.png}
  Predicted IC50 (in nM) for EpitopePrediction and MHCnuggets
  \caption{
    \richel{Used randomly simulated polypeptides, results are real}
  }
  \label{fig:ep_vs_mhcn}
\end{figure}

%\begin{table}[!htbp]
%  \input{ep_vs_mhcn/ep_vs_mhcn.latex}
%  \caption{
%    Correlation coefficients for different haplotypes
%    \richel{Used randomly simulated polypeptides, results are real}
%  }
%  \label{table:ep_vs_mhcn}
%\end{table}
...
Here we have simulated \richel{some, say 100} random peptides.
Per peptide, we measured the IC50 
per MHC-I (figure \label{fig:ic50s_per_haplotype_mhc_1})
and MHC-II  (figure \label{fig:ic50s_per_haplotype_mhc_2}) haplotype.

\begin{figure}[!htbp]
  \includegraphics[width=0.9\textwidth]{ic50s_per_haplotype/ic50s_per_haplotype_mhc_1.png}
  Predicted IC50s (in nM) per MHC-I haplotype
  \caption{
    \richel{Used randomly simulated polypeptides, results are real}
  }
  \label{fig:ic50s_per_haplotype_mhc_1}
\end{figure}

\begin{figure}[!htbp]
  \includegraphics[width=0.9\textwidth]{ic50s_per_haplotype/ic50s_per_haplotype_mhc_2.png}
  Predicted IC50s (in nM) per MHC-II haplotype
  \caption{
    \richel{Used randomly simulated polypeptides, results are real}
  }
  \label{fig:ic50s_per_haplotype_mhc_2}
\end{figure}
