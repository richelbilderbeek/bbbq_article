...
for TMHs and soluble protein regions. 
We obtained 1,129 gene names associated with the phrase 'membrane protein'.
These genes are linked to 4,811 protein isoforms, 
of which 2,568 are predicted to be transmembrane proteins (TMPs),
where the others are predicted to be membrane-associated proteins.
...
distribution.
We calculated the number of SNPs that are expected to be
found in TMHs by chance, $E(n_{SNP\_TMH})$, when including
and excluding soluble proteins (with symbols
$E(n_{SNP\_TMH}^{+})$ and $E(n_{SNP\_TMH}^{-})$ respectively).
The number of SNPs that are expected to be
found in TMHs by chance when including soluble proteins,
$E(n_{SNP\_TMH}^{+})$ 
equals the number of (all) SNPs, $n_{SNP}^{+} = 38,882$, multiplied by the fraction
of all proteins that are TMH, $f_{TMH}^{+} = 9.96\%$,
resulting in 
$E(n_{SNP\_TMH}^{+}) = n_{SNP}^{+} \cdot f_{TMH}^{+} \approx 3873$
The number of SNPs that are expected to be
found in TMHs by chance when excluding soluble proteins,
$E(n_{SNP\_TMH}^{-})$ 
...
The discrepancy between the two expectations
is caused by the distribution of SNPs over the soluble and
transmembrane proteins: 
there are 38,882 SNPs found in 4811 proteins (i.e. $\approx 8.08$ SNP per protein),
of which 21,738 SNPs in 2568 TMPs (i.e. $\approx 8.46$
SNP per TMP).

\richel{How many SNPs in TMHs?}
