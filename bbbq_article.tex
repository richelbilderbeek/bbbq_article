...
One interesting finding is that the MHC-I 
haplotype HLA-A*24:02 
present four times as much epitopes derived from SARS-CoV-2 TMHs
than expected by chance.
This may help the immune system to detect a SARS-CoV-2 infection earlier,
but we have not investigates if an effect can be found in clinical data.
...
by mass spectronomy \richel{Look up wording and ref in Bianchi et al 2017}.
...
human TMHs (\richel{Check}), thus we cannot be sure that
...
\subsection{Notes and/or to be removed}
...
\begin{itemize}
  ...
  \item $\mathcal{H}_{3}$: TMHs and soluble protein regions are equally conserved
    in a proteome
\end{itemize}
