...
of TMH predictions. 
Contemporary tools are the closed-source MemBrain (\cite{feng2020topology}) 
and the FOSS tool PureseqTM (\cite{wang2019efficient}).
In this study, we will use PureseqTM.
...
peptides (see Supmat for values \richel{Put these values in supmat}), were the original study defined
...
there is a 14% chance to find one haplotype 
that hints to have selection working upon it.
% > pbinom(q = 1, size = 13, prob = 0.05, lower.tail = FALSE)
% [1] 0.1354239

The chance to find 2 haplotypes that hint to have selection working upon it
is 2\%, which is below our significance level.
% > pbinom(q = 2, size = 13, prob = 0.05, lower.tail = FALSE)
% [1] 0.02450784
